...
全部累進同一張 39 格表，畫面上的 A 位置不影響結果，只有 B 決定預測期。回溯次數 `steps: 19`（使用者 2026-09-14 由 100 改 19）、備用列 `spareRows: 96`（16+96 = 112 列，可撐到回溯 100 次；回溯 19 次只用到備用列前 25 期）；CLI 與模組預設仍是 16，用 `--steps 7` 可對照。
