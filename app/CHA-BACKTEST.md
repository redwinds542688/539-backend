...
6. 「差數 同差法 統計表」（使用者 2026-09-14 指示）：上桿標定號碼加哪個九宮差落在上桿列（可能不只一個），對應下桿號碼就加同一個九宮差，
   ...
   2026-09-14 再改：進入 C 式差數就自動跑同差法填空白期（App 端包了 `switchCSubMode`），單擊差數與雙擊 Ai 在差數定位中都取消作用。以下為改前的敘述：**單擊「差數」鍵**開這張表（再單擊關），前二名（照 App 前幾名的定義：次數最高的兩個名次、同次數並列都算、次數必須大於 1、最多 15 顆；2026-09-14 由前三名改前二名；2026-09-16 起不再補第三名）用 App 原本的填空白格機制填進下方空白期；
