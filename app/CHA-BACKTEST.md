...
4. 統計表開出的同時，把最高排行前二名（跟空白期同定義：次數最高的兩個名次、並列全算、次數必須大於 1、最多 15 顆，2026-09-14 由 3 改 5 再改成前二名定義；2026-09-16 使用者指示前二名合計 2 顆以下時不再補第三名）直排顯示在左側 Ai 欄。
   > 2026-09-17 使用者指示：空白期（差數／定位／合匯）恢復「前二名，合計不足或等於 2 顆補第三名」（上限 539/天天樂 15、大樂透/六合彩 21）；同差法／合匯完成後 Ai 欄不再顯示「前二名以外、次數>0」的名單，Ai 欄只留給長按 Ai 的紀錄法（前二名、次數>1、最多 16）
...
全部累進同一張 39 格表，畫面上的 A 位置不影響結果，只有 B 決定預測期。回溯次數 `steps: 16`（使用者依序 100→19→6→42→16→100→16，2026-09-16 定 16）、備用列 `spareRows: 96`（16+96 = 112 列，回溯第 100 次差 6 的上桿往上搜 6 期剛好到備用列第 1 期）；CLI 與模組預設仍是 16，用 `--steps 7` 可對照。
