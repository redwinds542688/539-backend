...
4. 統計表開出的同時，把最高排行前二名（跟空白期同定義：次數最高的兩個名次、並列全算、次數必須大於 1、最多 15 顆，2026-09-14 由 3 改 5 再改成前二名定義；2026-09-16 使用者指示前二名合計 2 顆以下時不再補第三名）直排顯示在左側 Ai 欄。
   ...
   > 2026-09-19 使用者統一定調（規格第 55 條，取代第 53/54 條）：單擊差數／合匯／定位的 Ai 欄跟空白期名單無關——下桿上四期為參考，號碼 A 從 01 到 39（大樂透／六合彩 49）逐一取九宮拖牌（含 A 本身 9 顆），拖牌在四期裡命中 3 期以上就把 9 顆拖牌各記 1 次進統計表；全部跑完取最前面 3 名（並列全算、最多 16 顆）列在 Ai 欄，前四期開過的標「已開」。長按差數的對照表同步。長按 Ai 的紀錄法不受影響。
