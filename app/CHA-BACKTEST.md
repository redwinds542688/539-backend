...
全部累進同一張 39 格表，畫面上的 A 位置不影響結果，只有 B 決定預測期。回溯次數 `steps: 42`（使用者 2026-09-14 依序 100→19→6→42）、備用列 `spareRows: 96`（16+96 = 112 列，可撐到回溯 100 次；回溯 42 次只用到備用列前 48 期）；CLI 與模組預設仍是 16，用 `--steps 7` 可對照。
