...
4. 統計表開出的同時，把最高排行前二名（跟空白期同定義：次數最高的兩個名次、並列全算、次數必須大於 1、最多 15 顆，2026-09-14 由 3 改 5 再改成前二名定義；2026-09-16 使用者指示前二名合計 2 顆以下時不再補第三名）直排顯示在左側 Ai 欄。
   ...
   > 2026-09-17 再指示：空白期號碼「搭配九宮差」——九個九宮差各算「空白期號碼＋九宮差」落在下桿上一期的顆數，取最多的那個九宮差，把空白期號碼全部加上它顯示在 Ai 欄（跟下桿上一期一樣的號碼左上角標「已開」，其他不畫次數；全部沒命中則留空；最多 16 顆；紀錄法顯示中不搶）。命中數最多的九宮差有好幾個平手時，每個平手的九宮差都把空白期號碼加好，全部放進同一張統計（出現幾次計幾次），再取前二名（並列全算、合計 ≤2 顆補第三名）。差數／定位／合匯都適用（App 端 `cModeBlankShiftToAi`）
