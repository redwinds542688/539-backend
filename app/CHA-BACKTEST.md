...
4. 統計表開出的同時，把最高排行前二名（跟空白期同定義：次數最高的兩個名次、並列全算、次數必須大於 1、最多 15 顆，2026-09-14 由 3 改 5 再改成前二名定義；2026-09-16 使用者指示前二名合計 2 顆以下時不再補第三名）直排顯示在左側 Ai 欄。
   ...
   > 2026-09-20（第 76 條）：位置改成「有開的一二三期」＝顯示區最上面兩列（空白列不再畫）；被蓋住的開獎號碼存進 dataset，normalizedNum 代還原，統計與搜尋讀到的仍是原號碼。
   > 2026-09-20（第 77 條）：最強九宮差改成「所有連七號碼的九宮差命中次數加總後取最大」（整張表共用一個），表格三列＝連七號碼／最強九宮差（角標＝總次數）／號碼加上它的結果。
