...
固定框架（跟 App 畫面一致，回測全程不動）：
    ...
    備用列 = 顯示區上方再 32 期    rows[N-48 .. N-17]

targetIdx = 預測目標列（下桿位置）；預設 N = 空白第 1 列（App 第 17 列）

for t = 1 .. 16:                       # 從目標列的上一列往上回溯 16 次
    lowerIdx = targetIdx - t           # 目標第 17 列 → 回溯第 16..1 列；目標第 16 列 → 回溯第 15..0 列（第 0 列在備用列）
                                       # 這一期就是真實的果
...
2. sweep(rows[0..targetIdx), targetIdx) → 下桿放在目標列（預設空白第 1 列，App 第 17 列），上桿掃上方 6..1 列
...
CLI：`--target 17`（顯示列號，17 = 空白第 1 列）、`--predict [N]`、`--predict-mode top|field|condition`。
...
回測起點跟著預測目標列走（目標列 − 1 往上 16 次），目標在第 16 列時最後一次回溯落在第 0 列，也就是備用列第 1 列，仍然夠用。
