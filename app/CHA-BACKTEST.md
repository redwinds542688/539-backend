...
4. 統計表開出的同時，把最高排行前二名（跟空白期同定義：次數最高的兩個名次、並列全算、次數必須大於 1、最多 15 顆；2026-09-14 由 3 改 5 再改成前二名定義）直排顯示在左側 Ai 欄
...
6. 「差數 同差法 統計表」（使用者 2026-09-14 指示）：上桿標定號碼加哪個九宮差落在上桿列（可能不只一個），對應下桿號碼就加同一個九宮差，
   ...
   **單擊「差數」鍵**開這張表（再單擊關），前二名（照 App 前幾名的定義：次數最高的兩個名次、同次數並列都算、次數必須大於 1、最多 15 顆；2026-09-14 由前三名改前二名）用 App 原本的填空白格機制填進下方空白期；
...
全部累進同一張 39 格表，畫面上的 A 位置不影響結果，只有 B 決定預測期。回溯次數 `steps: 16`（使用者 2026-09-14 依序 100→19→6→42→16）、備用列 `spareRows: 96`（16+96 = 112 列，可撐到回溯 100 次；回溯 16 次只用到備用列前 22 期）；CLI 與模組預設仍是 16，用 `--steps 7` 可對照。
