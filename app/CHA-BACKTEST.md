...
6. 「差數 同差法 統計表」（使用者 2026-09-14 指示）：上桿標定號碼加哪個九宮差落在上桿列（可能不只一個），對應下桿號碼就加同一個九宮差，
   ...
   搜索期原本 3、4、5 各跑一次累進同一張表（使用者 2026-09-14）；2026-09-16 改成用畫面「搜期」設定：搜期 3~6 改打勾（初始全勾，定位/差數各一份），有打勾的搜期各跑一次、累進同一張表，畫面標定用的搜期＝最大打勾值（App 端 `cModeCheckedSpans`、`runChaSameAnimatedSweep`、`chaSameOffsetAllSpans`）。
