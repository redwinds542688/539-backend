...
5. 統計表開著時移動上下桿：拖曳開始就把 39 格歸零、Ai 欄清空（`window.__chaRecordsReset`），
   桿子落定後（App 原本重算掃描表的同一個地方）自動再跑一次長按 Ai 的功能。

App 目前用 `sweepAll: true`（使用者 2026-09-14 指示「上下桿差 6 到 1 都要加入統計」）：上桿差 1 到差 6 各跑一次紀錄法，
全部累進同一張 39 格表，畫面上的 A 位置不影響結果，只有 B 決定預測期。回溯次數 `steps: 3`（使用者 2026-09-14 指示「回溯 16 改成 7」再改成 3，
回溯 N-1 … N-3）；CLI 與模組預設仍是 16，用 `--steps 7` 可對照。
